...
Panel {
  ...
  readonly property var profileNames: ["Work", "Home", "Travel", "Laptop only"]
  readonly property var profileCommands: ["work", "home", "travel", "laptop"]
  ...
  function moveCursorH(delta) {
    if (focusSection !== "scale" && focusSection !== "profiles") return
    var length = focusSection === "scale" ? scaleValues.length : profileCommands.length
  ...
  function activateCursor() {
    if (focusSection === "profiles" && selectedIndex >= 0 && selectedIndex < profileCommands.length) {
      setProfile(profileCommands[selectedIndex])
      return
    }
  ...
  function setProfile(profile) {
    if (!profile) return
    actionProc.command = ["bash", "-lc", "$HOME/.config/hypr/scripts/monitor-profile.sh " + profile]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  Process {
    id: profileStateProc
    command: ["bash", "-lc", "$HOME/.config/hypr/scripts/monitor-profile.sh active"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var value = String(text || "").trim()
        root.activeProfile = value || "unknown"
      }
    }
  }
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        ...
        if (dy !== 0) root.moveCursor(dy)
          ...
          else if (root.focusSection === "scale" || root.focusSection === "profiles") root.moveCursorH(dx)
        }
      }
      ...
      ScrollView {
        ...
        Column {
          ...
          Column {
            ...
            Item {
              width: parent.width
              implicitHeight: Math.max(layoutHeader.implicitHeight, layoutActiveProfile.implicitHeight)
              ...
              Text {
                id: layoutActiveProfile
                property int activeIndex: root.profileCommands.indexOf(root.activeProfile)
                text: activeIndex >= 0 ? root.profileNames[activeIndex].toUpperCase() : "UNKNOWN"
                color: Qt.darker(root.bar.foreground, 1.4)
                font.family: root.bar.fontFamily
                font.pixelSize: Style.font.caption
                font.bold: true
            ...
            Grid {
              id: profileRow
              width: parent.width
              columns: root.profileNames.length
              spacing: Style.spacing.xs

              readonly property real cellWidth: root.profileNames.length > 0
  ...
  component ProfilePill: Button {
    ...
    verticalPadding: Style.spacing.controlPaddingY
    bordered: true
    active: root.activeProfile === root.profileCommands[profileIndex]
    hasCursor: root.cursorActive && root.focusSection === "profiles" && root.selectedIndex === profileIndex

    onClicked: root.setProfile(root.profileCommands[profileIndex])
    onHovered: function(isHovered) {
      if (!isHovered || root.reflowingText) return
      root.cursorActive = true
