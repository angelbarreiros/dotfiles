...
Panel {
  id: root
  moduleName: "demper.power"
  ipcTarget: "demper.power"
  ...
  function updateProfiles(raw) {
    var parsed = Model.parseTuxedoProfiles(raw, profileIndex)
    if (parsed.profiles.length === 0) return
    profiles = parsed.profiles
    profileIndex = parsed.profileIndex
    if (opened && !cursorActive) {
      var idx = Model.profileIndexById(profiles, activeProfile)
      if (idx >= 0) profileIndex = idx
    }
  }

  function updateActiveProfile(raw) {
    var parsed = Model.parseTuxedoActiveProfile(raw)
    if (!parsed.id) return
    activeProfile = parsed.id
    if (opened && !cursorActive) {
      var idx = Model.profileIndexById(profiles, activeProfile)
      if (idx >= 0) profileIndex = idx
    }
  ...
  function setProfile(profile) {
    ...
    actionProc.command = ["bash", "-lc", "$HOME/.config/hypr/scripts/tuxedo-profile.sh set " + Util.shellQuote(String(profile.id || ""))]
  ...
  Process {
    id: profilesProc
    command: ["bash", "-lc", "$HOME/.config/hypr/scripts/tuxedo-profile.sh list"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateProfiles(text) }
  }

  Process {
    id: activeProfileProc
    command: ["bash", "-lc", "$HOME/.config/hypr/scripts/tuxedo-profile.sh active"]
          ...
          Grid {
            ...
            Repeater {
              ...
              Button {
                ...
                iconText: root.profileIcon(String(modelData.name || modelData.id || ""))
                ...
                bordered: true
                active: root.activeProfile === modelData.id
