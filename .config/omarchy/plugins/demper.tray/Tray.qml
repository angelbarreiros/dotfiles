...
BarWidget {
  ...
  // The bar slot has its own drag/click MouseArea above widget contents.
  // Keep track of the tray item under the pointer so a slot-level click can
  // still activate the correct system-tray item.
  ...
  function triggerPress(button) {
    ...
    if (button === Qt.MiddleButton) {
      ...
      if (item.modelData.onlyMenu) {
        item.displayMenu({ x: item.width / 2, y: item.height / 2 })
      } else {
        activateTrayItem(item.modelData)
      }
    }
  }
  ...
  function activateTrayItem(item) {
    item.activate()
    Util.execDetached("bash ~/.config/hypr/scripts/launch-or-focus-tray-app.sh "
      + Util.shellQuote(String(item.id || "")) + " "
      + Util.shellQuote(String(item.title || "")) + " "
      + Util.shellQuote(String(item.tooltipTitle || "")))
  }
  ...
  component TrayItem: Item {
    ...
    function syncClickRegistration() {
      if (registeredBar && registeredBar.unregisterClickTarget)
        registeredBar.unregisterClickTarget(trayItemRoot)
      registeredBar = trayBar
      if (registeredBar && registeredBar.registerClickTarget)
        registeredBar.registerClickTarget(trayItemRoot)
    }
    ...
    function triggerPress(button) {
      if (button === Qt.MiddleButton) {
        ...
        if (modelData.onlyMenu)
          displayMenu({ x: width / 2, y: height / 2 })
        else
          root.activateTrayItem(modelData)
      }
    }

    onTrayBarChanged: syncClickRegistration()
    ...
    MouseArea {
      ...
      cursorShape: Qt.PointingHandCursor
      onEntered: {
        root.hoveredTrayItem = trayItemRoot
        if (root.bar) root.bar.showTooltip(trayItemRoot, root.trayTooltip(modelData))
      }
      onExited: {
        if (root.hoveredTrayItem === trayItemRoot) root.hoveredTrayItem = null
      ...
      onClicked: function(mouse) {
        if (mouse.button === Qt.RightButton) {
          ...
          root.activateTrayItem(trayItemRoot.modelData)
        }
      }
